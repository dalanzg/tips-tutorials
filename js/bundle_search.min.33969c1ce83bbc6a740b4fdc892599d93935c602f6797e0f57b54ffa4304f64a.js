...
function processResultData(searchResults){$results=[];console.log("Search Results: "+searchResults);var resultsCount=0,results="";searchResults.forEach(function(result){var loaded_data=JSON.parse(sessionStorage.getItem("actualData"));var item=loaded_data[result.ref];var result=populateResultContent($resultTemplate.html(),item);resultsCount++;results+=result;});if(showLoader){toggleLoadingClass();}
populateResultsString(resultsCount);showSearchResults(results);}
function showSearchResults(results){$resultsPlaceholder.html(results);}
function populateResultContent(html,item){html=injectContent(html,item.title,'##title##');html=injectContent(html,item.description,'##description##');html=injectContent(html,item.date,'##date##');html=injectContent(html,item.author,'##author##');html=injectContent(html,item.tags,'##tags##');html=injectContent(html,item.image,'##image##');html=injectContent(html,item.url,'##url##');html=injectContent(html,item.content,'##content##');return html;}
